"""
onemoney Import Schema
=====================
Pydantic v2 модели для файла импорта данных в приложение onemoney.

Формат файла: JSON
Кодировка: UTF-8

Использование:
    from schema import onemoneyImport, validate_file

    data = validate_file("my_data.json")
    # или напрямую:
    data = onemoneyImport.model_validate_json(json_string)
"""
...
# ---------------------------------------------------------------------------
# onemoneyImport — корневая модель
# ---------------------------------------------------------------------------

class onemoneyImport(BaseModel):
    """
    Корневая модель файла импорта onemoney.
    ...
    @model_validator(mode="after")
    def validate_refs(self) -> "onemoneyImport":
...
def validate_file(path: str | Path) -> onemoneyImport:
    """
    ...
    Returns:
        Провалидированная модель onemoneyImport
    ...
    return onemoneyImport.model_validate_json(content)


def summary(data: onemoneyImport) -> str:
    ...
    lines = [
        f"onemoney Import v{data.version}",
