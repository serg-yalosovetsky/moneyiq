...
### Расход → перемещение на свой счёт

Обратное преобразование к «перевод → расход». Нужно для переводов на собственный
брокерский/кошельковый счёт (IBKR, Payoneer, наличные): банк присылает их расходом,
хотя деньги не потрачены.

- В `QuickCategoryPickerSheet` последним разделом идёт «Переказ» (`transferSection`) со
  счетами; счёт-источник (`tx.accountId`) из списка исключён — перевод сам на себя
  бессмысленен. Поиск фильтрует и счета: запрос «IBKR» оставляет только нужный.
- Выбор счёта показывает `tx_convert_move_*` и после подтверждения ставит
  `moveToAccountId`; категория снимается (`categoryId = null`).
- `TransactionsListViewModel.updateTransaction(moveToAccountId = …)` меняет тип на
  `TRANSFER`, ставит `toAccountId` и отправляет правку с `to_account_id` на mono-flow.

**Rule:** Шапка листа считает состояние от `movedToAccount` (счёт, выбранный в этой
сессии), а не только от `tx`. Без этого после подтверждения «Перемістити» шапка
продолжала показывать старую категорию и тип «Витрата» — человеку казалось, что
ничего не произошло, потому что сохранение случается лишь при закрытии листа.
